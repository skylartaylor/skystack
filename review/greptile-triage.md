...
```bash
REMOTE_SLUG=$(browse/bin/remote-slug 2>/dev/null || ~/.claude/skills/skystack/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
PROJECT_HISTORY="$HOME/.skystack/projects/$REMOTE_SLUG/greptile-history.md"
```
...
```bash
REMOTE_SLUG=$(browse/bin/remote-slug 2>/dev/null || ~/.claude/skills/skystack/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
mkdir -p "$HOME/.skystack/projects/$REMOTE_SLUG"
mkdir -p ~/.skystack
```

Append one line per triage outcome to **both** files (per-project for suppressions, global for retro):
- `~/.skystack/projects/$REMOTE_SLUG/greptile-history.md` (per-project)
- `~/.skystack/greptile-history.md` (global aggregate)
