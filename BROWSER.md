...
This document covers the command reference and internals of skystack's headless browser.
...
skystack's browser is a compiled CLI binary that talks to a persistent local Chromium daemon over HTTP. The CLI is a thin client — it reads a state file, sends a command, and prints the response to stdout. The server does the real work via [Playwright](https://playwright.dev/).
...
Each server session generates a random UUID as a bearer token. The token is written to the state file (`.skystack/browse.json`) with chmod 600. Every HTTP request must include `Authorization: Bearer <token>`. This prevents other processes on the machine from controlling the browser.
...
- Console: `.skystack/browse-console.log`
- Network: `.skystack/browse-network.log`
- Dialog: `.skystack/browse-dialog.log`
...
Each workspace gets its own isolated browser instance with its own Chromium process, tabs, cookies, and logs. State is stored in `.skystack/` inside the project root (detected via `git rev-parse --show-toplevel`).

| Workspace | State file | Port |
|-----------|------------|------|
| `/code/project-a` | `/code/project-a/.skystack/browse.json` | random (10000-60000) |
| `/code/project-b` | `/code/project-b/.skystack/browse.json` | random (10000-60000) |
...
| `BROWSE_STATE_FILE` | `.skystack/browse.json` | Path to state file (CLI passes to server) |
...
| **skystack browse** | **~3s** | **~100-200ms** | **0 tokens** (plain text stdout) |

The context overhead difference compounds fast. In a 20-command browser session, MCP tools burn 30,000-40,000 tokens on protocol framing alone. skystack burns zero.
...
skystack skips all of this. Compiled binary. Plain text in, plain text out. No protocol. No schema. No connection management.
...
| File | Role |
|------|------|
| `browse/src/cli.ts` | Entry point. Reads `.skystack/browse.json`, sends HTTP to the server, prints response. |
...
1. Push your branch
2. Pull in the skill directory: `cd ~/.claude/skills/skystack && git pull`
