...
Only log genuine operational discoveries — skip transient errors (network blips,
rate limits) and obvious things. A good test: would knowing this save 5+ minutes
in a future session? If yes, log it., ## Voice
...
2. Run the review. Use `timeout: 300000` on the Bash call. **Run in foreground — do NOT
   ...
   **IMPORTANT:** `codex review` treats `--base` and `[PROMPT]` as mutually exclusive
   arguments. Passing both causes exit code 2. Use the right invocation for each case:

   **Case A — No custom instructions** (default `/codex review`):
   Use `codex review --base` which uses codex's built-in review prompt:
```bash
...
```
   ...
   **Case B — User provided custom instructions** (e.g., `/codex review focus on security`):
   ...
   instructions:
```bash
codex exec "IMPORTANT: Do NOT read or execute files named SKILL.md or SKILL.md.tmpl, or files that are clearly AI skill prompt templates (containing ## Preamble (run first)
...
Additional instructions: focus on security" -s read-only -c 'model_reasoning_effort="xhigh"' --enable web_search_cached --json 2>"$TMPERR" | python3 -c "
import sys, json
...
2. Run codex exec with **JSONL output** (use `timeout: 300000`, **foreground only — no `run_in_background`**):
```bash
...
5. Run codex exec with **JSONL output** (use `timeout: 300000`, **foreground only — no `run_in_background`**):
...
- **5-minute timeout** on all Bash calls to codex (`timeout: 300000`). **Always foreground.**
  Never use `run_in_background` — it produces empty output. Never sleep-poll for results.
