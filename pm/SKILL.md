...
### 1a. Understand the codebase context
...
Write a concise feature spec and present it to the user for approval.
...
3. **Design recommendations** — Based on competitive and UX research:
   - Recommended display pattern (with rationale from research)
   - Component/library choices (tailored to detected stack)
   - Key interaction details (animations, transitions, feedback)
4. **Accessibility plan** — The stack-specific checklist from Phase 1d
5. **Edge cases** — What happens with 0 data? 1 data point? 10,000? No permission? Offline?
6. **Non-goals** — What this feature explicitly does NOT do (prevents scope creep)

**Present the spec via AskUserQuestion:**

"Here's the feature spec for [feature]. Review the approach and let me know:
...
RECOMMENDATION: Always recommend A unless you have genuine concerns. Include a one-line
rationale. If you think the scope should be expanded or reduced, say so with reasoning.
...
Keep the plan grounded in the codebase patterns discovered in Phase 1a. Don't introduce
...
5. **Two checkpoints, no more.** Spec approval (Phase 2) and ship decision (Phase 6).
   Everything else runs autonomously. The user hired a PM, not an assistant that asks
   permission for every line of code.
