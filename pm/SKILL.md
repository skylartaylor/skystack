...
## Cross-Model Code Review — independent second opinion

After verification passes, get a final review from both Claude (/review specialists)
and Codex before presenting to the user. This catches issues the per-batch reviewers
might miss when looking at the full feature diff.
...
1. Print: `running cross-model review (review + codex in parallel)...`

2. Dispatch these **in parallel** (single message with 1 Agent call + 1 Bash call):

   **Agent: /review subagent** — dispatch a `general-purpose` subagent with `model: "opus"`:

   Tell the subagent to:
   - First, save the feature diff to a temp file using the build start SHA, stripping
     lock files and minified assets via git pathspec excludes:
     ```
     NOISE_EXCLUDES=(
       ':(exclude)**/bun.lock' ':(exclude)**/package-lock.json'
       ':(exclude)**/yarn.lock' ':(exclude)**/pnpm-lock.yaml'
       ':(exclude)**/Cargo.lock' ':(exclude)**/go.sum'
       ':(exclude)**/poetry.lock' ':(exclude)**/Pipfile.lock'
       ':(exclude)**/flake.lock' ':(exclude)**/Gemfile.lock'
       ':(exclude)**/composer.lock'
       ':(exclude)**/*.min.js' ':(exclude)**/*.min.css'
       ':(exclude)**/*.bundle.js' ':(exclude)**/*.map'
     )
     git diff <build_start_sha>..HEAD -- . "${NOISE_EXCLUDES[@]}" > /tmp/pm-review-diff-$$.txt
     ```
   ...
   - Each specialist receives the diff file path, the review checklist
     (`~/.claude/skills/skystack/review/checklist.md` or `.claude/skills/skystack/review/checklist.md`),
     and the detected stack
   - Each specialist returns findings in the FINDINGS format (SEVERITY, CATEGORY, LOCATION, ISSUE, FIX)
   ...
   **Scope the review to only this feature's changes.** Use the build start SHA (recorded
   ...
   git diff "$BUILD_START_SHA"..HEAD -- . "${NOISE_EXCLUDES[@]}" > "$DIFF_FILE"
   DIFF_STATS=$(git diff --stat "$BUILD_START_SHA"..HEAD -- . "${NOISE_EXCLUDES[@]}" | tail -1)
...
1. Print: `Codex not installed — running Claude review only.`

2. Dispatch only the /review subagent (same as above, without the parallel codex call).

3. Skip the cross-model synthesizer. Instead, use the /review findings directly:
   - If no CRITICAL findings: proceed to Present & Publish with clean recommendation.
   - If CRITICAL findings exist: dispatch a fix subagent for auto-fixable items,
     carry remaining findings to Present & Publish.
...
- Cross-model review findings (if any remain unfixed) — present findings-first, source-second
