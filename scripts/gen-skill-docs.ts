...
function resolveVoiceGuide(_tmplPath: string): string {
  return generateVoiceDirective();
}
...
for (const skill of CLAUDE_SKILLS) {
  ...
  if (relOutput !== skill.claudeOutput) {
    throw new Error(
      `Catalog output mismatch for ${skill.name}: expected ${skill.claudeOutput}, got ${relOutput}`,
    );
  }
