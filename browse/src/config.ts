 ...
 *   2. git rev-parse --show-toplevel → projectDir/.skystack/
...
export function resolveConfig(
  ...
  if (env.BROWSE_STATE_FILE) {
    ...
    projectDir = path.dirname(stateDir); // parent of .skystack/
  } else {
    projectDir = getGitRoot() || process.cwd();
    stateDir = path.join(projectDir, '.skystack');
...
/**
 * Create the .skystack/ state directory if it doesn't exist.
...
export function ensureStateDir(config: BrowseConfig): void {
  ...
  // Ensure .skystack/ is in the project's .gitignore
  const gitignorePath = path.join(config.projectDir, '.gitignore');
  try {
    const content = fs.readFileSync(gitignorePath, 'utf-8');
    if (!content.match(/^\.skystack\/?$/m)) {
      const separator = content.endsWith('\n') ? '' : '\n';
      fs.appendFileSync(gitignorePath, `${separator}.skystack/\n`);
    }
  } catch (err: any) {
    if (err.code !== 'ENOENT') {
