/**
 * Tests for bin/skystack-update-check bash script.
 *
 * Uses Bun.spawnSync to invoke the script with temp dirs and
 * SKYSTACK_DIR / SKYSTACK_STATE_DIR / SKYSTACK_REMOTE_URL env overrides
 * for full isolation.
 */
...
const SCRIPT = join(import.meta.dir, '..', '..', 'bin', 'skystack-update-check');

let skystackDir: string;
let stateDir: string;
...
function run(extraEnv: Record<string, string> = {}, args: string[] = []) {
  const result = Bun.spawnSync(['bash', SCRIPT, ...args], {
    env: {
      ...process.env,
      SKYSTACK_DIR: skystackDir,
      SKYSTACK_STATE_DIR: stateDir,
      SKYSTACK_REMOTE_URL: `file://${join(skystackDir, 'REMOTE_VERSION')}`,
      ...extraEnv,
    },
    stdout: 'pipe',
    stderr: 'pipe',
...
beforeEach(() => {
  skystackDir = mkdtempSync(join(tmpdir(), 'skystack-upd-test-'));
  stateDir = mkdtempSync(join(tmpdir(), 'skystack-state-test-'));
  // Link real skystack-config so update_check config check works
  const binDir = join(skystackDir, 'bin');
  mkdirSync(binDir);
  symlinkSync(join(import.meta.dir, '..', '..', 'bin', 'skystack-config'), join(binDir, 'skystack-config'));
});

afterEach(() => {
  rmSync(skystackDir, { recursive: true, force: true });
...
describe('skystack-update-check', () => {
  ...
  test('exits 0 with no output when VERSION file is empty', () => {
    writeFileSync(join(skystackDir, 'VERSION'), '');
    const { exitCode, stdout } = run();
    expect(exitCode).toBe(0);
    expect(stdout).toBe('');
  });
  ...
  test('re-checks when UP_TO_DATE cache version does not match local', () => {
    ...
    writeFileSync(join(skystackDir, 'REMOTE_VERSION'), '0.5.0\n');
  ...
  test('treats invalid remote response as up to date', () => {
    writeFileSync(join(skystackDir, 'VERSION'), '0.3.3\n');
    writeFileSync(join(skystackDir, 'REMOTE_VERSION'), '<html>404 Not Found</html>\n');
  ...
  test('exits silently when remote URL is unreachable', () => {
    ...
    const { exitCode, stdout } = run({
      SKYSTACK_REMOTE_URL: 'file:///nonexistent/path/VERSION',
    });
    expect(exitCode).toBe(0);
    expect(stdout).toBe('');
    const cache = readFileSync(join(stateDir, 'last-update-check'), 'utf-8');
  ...
  test('creates state dir if it does not exist', () => {
    ...
    const { exitCode } = run({ SKYSTACK_STATE_DIR: newStateDir });
    expect(exitCode).toBe(0);
  ...
  test('exits 0 with real project VERSION and unreachable remote', () => {
    ...
    writeFileSync(join(skystackDir, 'VERSION'), version + '\n');
