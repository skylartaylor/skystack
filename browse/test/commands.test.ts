...
describe('CLI server script resolution', () => {
  test('prefers adjacent browse/src/server.ts for compiled project installs', () => {
    const root = fs.mkdtempSync('/tmp/skystack-cli-');
    const execPath = path.join(root, '.claude/skills/skystack/browse/dist/browse');
    const serverPath = path.join(root, '.claude/skills/skystack/browse/src/server.ts');
