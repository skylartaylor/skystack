/**
 * Tests for bin/skystack-config bash script.
 *
 * Uses Bun.spawnSync to invoke the script with temp dirs and
 * SKYSTACK_STATE_DIR env override for full isolation.
 */
...
const SCRIPT = join(import.meta.dir, '..', '..', 'bin', 'skystack-config');
...
beforeEach(() => {
  stateDir = mkdtempSync(join(tmpdir(), 'skystack-config-test-'));
});
...
describe('skystack-config', () => {
  ...
  test('set creates state dir if missing', () => {
    ...
    const { exitCode } = run(['set', 'foo', 'bar'], { SKYSTACK_STATE_DIR: nestedDir });
    expect(exitCode).toBe(0);
