...
mkdir -p ~/.skystack/projects/$SLUG/ceo-plans
```

Before writing, check for existing CEO plans in the ceo-plans/ directory. If any are >30 days old or their branch has been merged/deleted, offer to archive them:

```bash
mkdir -p ~/.skystack/projects/$SLUG/ceo-plans/archive
# For each stale plan: mv ~/.skystack/projects/$SLUG/ceo-plans/{old-plan}.md ~/.skystack/projects/$SLUG/ceo-plans/archive/
```

Write to `~/.skystack/projects/$SLUG/ceo-plans/{date}-{feature-slug}.md` using this format:
...
```
NOTE: These represent human-team implementation hours. With CC + skystack,
...
* **Effort estimate:** S/M/L/XL (human team) → with CC+skystack: S→S, M→S, L→M, XL→L
* **Priority:** P1/P2/P3
...
echo '{"skill":"plan-ceo-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"MODE"}' >> ~/.skystack/projects/$SLUG/$BRANCH-reviews.jsonl
```
...
- **B)** Keep in `~/.skystack/projects/` only (local, personal reference)
- **C)** Skip
