...
| Mode | full | `--regression .skystack/qa-reports/baseline.json` |
...
1. **Project-scoped test plans:** Check `~/.skystack/projects/` for recent `*-test-plan-*.md` files for this repo
   ```bash
   eval $(~/.claude/skills/skystack/bin/skystack-slug 2>/dev/null)
   ls -t ~/.skystack/projects/$SLUG/*-test-plan-*.md 2>/dev/null | head -1
   ```
...
- Include full attribution comment:
  ...
  // Report: .skystack/qa-reports/qa-report-{domain}-{date}.md
  ```
