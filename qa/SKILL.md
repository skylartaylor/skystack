...
After **every fix** (not every 5 — every single one), compute the WTF-likelihood:
...
**Continuous tracking:** Print the current WTF% in the commit message suffix:
`fix(qa): ISSUE-NNN — short description [WTF: 8%]`

**Hard gate at 20%:** When WTF exceeds 20%, STOP immediately. Show the user:
- Fixes completed so far (count + commit SHAs)
- Current WTF score and which signals contributed
- Remaining issues by severity

Then use AskUserQuestion:
- Question: "WTF-likelihood hit {N}%. Should I keep going?"
- A) Continue — I trust the fixes so far
- B) Stop here — review what's been done
- C) Revert last fix and stop

**Hard cap: 50 fixes.** After 50 fixes, stop regardless of remaining issues or WTF score.
Print: "Hit the 50-fix cap. Remaining {N} issues deferred to TODOS.md."
