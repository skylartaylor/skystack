...
This document explains **why** skystack is built the way it is. For setup and commands, see CLAUDE.md. For contributing, see CONTRIBUTING.md.
...
The server writes `.skystack/browse.json` (atomic write via tmp + rename, mode 0o600):
...
Cookies are the most sensitive data skystack handles. The design:

1. **Keychain access requires user approval.** First cookie import per browser triggers a macOS Keychain dialog. The user must click "Allow" or "Always Allow." skystack never silently accesses credentials.
...
3. **Database is read-only.** skystack copies the Chromium cookie DB to a temp file (to avoid SQLite lock conflicts with the running browser) and opens it read-only. It never modifies your real browser's cookie database.
...
```
Browser events → CircularBuffer (in-memory) → Async flush to .skystack/*.log
```
...
```bash
jq '.tests[] | select(.exit_reason == "timeout") | .last_tool_call' ~/.skystack-dev/evals/_partial-e2e.json
```
...
`eval:compare` diffs two eval runs. `eval:summary` aggregates stats across all runs in `~/.skystack-dev/evals/`.
