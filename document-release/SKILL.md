...
```bash
find . -maxdepth 2 -name "*.md" -not -path "./.git/*" -not -path "./node_modules/*" -not -path "./.skystack/*" -not -path "./.context/*" | sort
```
...
(adapt to whatever project you're in — these are not skystack-specific):
...
```bash
gh pr view --json body -q .body > /tmp/skystack-pr-body-$$.md
```
...
```bash
gh pr edit --body-file /tmp/skystack-pr-body-$$.md
```

5. Clean up the tempfile:

```bash
rm -f /tmp/skystack-pr-body-$$.md
```
