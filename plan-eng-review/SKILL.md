...
Write to `~/.skystack/projects/{slug}/{user}-{branch}-test-plan-{datetime}.md`:
...
eval $(~/.claude/skills/skystack/bin/skystack-slug 2>/dev/null)
mkdir -p ~/.skystack/projects/$SLUG
echo '{"skill":"plan-eng-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"MODE"}' >> ~/.skystack/projects/$SLUG/$BRANCH-reviews.jsonl
```
